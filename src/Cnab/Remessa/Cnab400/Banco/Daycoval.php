...
class Daycoval extends AbstractRemessa implements RemessaContract
{
    ...
    const INSTRUCAO_PROTESTAR_VENC_XX = '00';
    ...
    const INSTRUCAO_VALOR_SOMA_MORA = '00';
    ...
    const INSTRUCAO_DEVOL_VENC_XX = '00';
    ...
    protected function header()
    {
        ...
        $this->add(38, 38, $this->getContaDv() ?: CalculoDV::daycovalContaCorrente($this->getAgencia(), $this->getContaDv()));*/
		$this->add(27, 46, Util::formatCnab('X', $this->getCodigoCliente(), 20));
        //$this->add(39, 46, '');
        $this->add(47, 76, Util::formatCnab('X', $this->getBeneficiario()->getNome(), 30));
        $this->add(77, 79, $this->getCodigoBanco());
        $this->add(80, 94, Util::formatCnab('X', 'Banco Daycoval S.A.', 15));
    ...
    public function addBoleto(BoletoContract $boleto)
    {
        ...
        $this->add(29, 29, $this->getContaDv() ?: CalculoDV::daycovalContaCorrente($this->getAgencia(), $this->getContaDv()));*/
        ...
        $this->add(71, 83, Util::formatCnab('X', '', 13, 2));
        ...
        $this->add(382, 385, '');
        $this->add(386, 391, $boleto->getJurosApos() === false ? '      ' : $boleto->getDataVencimento()->copy()->addDays($boleto->getJurosApos())->format('dmy'));
        $this->add(392, 393, Util::formatCnab('9', $boleto->getDiasProtesto($boleto->getDiasBaixaAutomatica()), 2));
        $this->add(394, 394, '0');
        $this->add(395, 400, Util::formatCnab('9', $this->iRegistros + 1, 6));

        // Verifica multa
		//print_r($boleto->getNotaFiscal());
        if ($boleto->getNotaFiscal()->getDocumento() > 0) {
            // Inicia uma nova linha de detalhe e marca com a atual de edição
            $this->iniciaDetalhe();
            // Campo adicional para a multa
            $this->add(1, 1, 4); // Adicional Multa
            $this->add(2, 16, Util::formatCnab('X', Util::onlyNumbers($boleto->getNotaFiscal()->getDocumento()), 15)); // Numero nota fiscal
            $this->add(17, 29, Util::formatCnab('9', Util::nFloat($boleto->getNotaFiscal()->getValorNota(), 2), 13)); // valor nota fiscal
            $this->add(30, 37, date('dmY',strtotime($boleto->getNotaFiscal()->getDataEmissao())));
            $this->add(38, 81, $boleto->getNotaFiscal()->getChaveAcesso());
			$this->add(82, 394, '');
            $this->add(395, 400, Util::formatCnab('9', $this->iRegistros + 1, 6));
        }
