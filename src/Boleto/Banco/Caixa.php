...
class Caixa  extends AbstractBoleto implements BoletoContract
{
    ...
    {				//$this->getNossoNumero()
        return substr($this->getNossoNumero(), 0, 2).'/'.substr($this->getNossoNumero(), 2, 19). '-' . CalculoDV::cefNossoNumero($this->getNossoNumero());
    }
     ...
     * @throws \Exception
     */
    protected function getCampoLivre() // esse é quando o codigo do beneficiario é 7 digitos
    {
        if ($this->campoLivre) {
    ...
    public static function parseCampoLivre($campoLivre) {
        return [
            ...
            'nossoNumero' => substr($campoLivre, 7, 3) . substr($campoLivre, 11, 3) . substr($campoLivre, 15, 9),
            'nossoNumeroDv' => substr($campoLivre, 23, 1),
            'nossoNumeroFull' => substr($campoLivre, 7, 3) . substr($campoLivre, 11, 3) . substr($campoLivre, 15, 9),
        ];
    }
}
