...
abstract class AbstractBoleto implements BoletoContract
{
    ...
    public function getMoraDia()
    {
        ...
        if ($this->getJuros() > 0) {			
			//echo $this->getValor()." ".$this->getJuros()."\n";
	...
	public function setNotaFiscal($notafiscal){
		Util::addNotaFiscal($this->notafiscal, $notafiscal);
        return $this;		
	}
    /**
    ...
    public function getCodigoBarras()
    {
		...
		//echo $this->campoCodigoBarras = substr($codigo, 0, 4) . $dv . substr($codigo, 4);
        return $this->campoCodigoBarras = substr($codigo, 0, 4) . $dv . substr($codigo, 4);
    }

    public function getCodigoBarras2()
    {
        if (!empty($this->campoCodigoBarras)) {
            return $this->campoCodigoBarras;
        }
    ...
    public function toArray()
    {
        return array_merge(
            [
                ...
                'codigo_barras2' => $this->getCodigoBarras2(),
                'beneficiario' => [
				...
				'notafiscal' =>
                    $this->getNotaFiscal()
                        ? [
                        'documento' => $this->getNotaFiscal()->getDocumento(),
                        'valornota' => $this->getNotaFiscal()->getValorNota(),
						'dataemissao' => $this->getNotaFiscal()->getDataEmissao(),
						'chaveacesso' => $this->getNotaFiscal()->getChaveAcesso(),                        
                    ]
                        : [],			 
